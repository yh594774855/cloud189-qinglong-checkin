...
```bash
ql repo https://github.com/yh594774855/cloud189-qinglong-checkin.git "ql_cloud189_checkin|src|accounts|package|README|doc" "" "node_modules|.token|.logs"
```
...
TY_PASSWORD_1=<password_1>
TY_USERNAME_2=<phone_2>
TY_PASSWORD_2=<password_2>
```
...
| `WECOM_BOT_KEY` / `WECOM_BOT_TELPHONE` | 企业微信群机器人推送 |
| `WX_PUSHER_APP_TOKEN` / `WX_PUSHER_UID` | WxPusher 推送 |
| `PUSH_PLUS_TOKEN` | PushPlus 推送 |
| `BARK_KEY` / `BARK_SERVER` | Bark 推送 |
| `SHOWDOC_KEY` | ShowDoc 推送 |

### 安全注意

账号密码只放在青龙环境变量或 GitHub Secrets 中。不要提交 `.env`、`.token`、`.logs`、截图、运行日志到公开仓库。
